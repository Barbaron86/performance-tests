...
open_debit_card_account_payload_response = httpx.post(
    'http://localhost:8003/api/v1/accounts/open-debit-card-account',
    json=open_debit_card_account_payload
)
open_debit_card_account_payload_response_data = open_debit_card_account_payload_response.json()
print(open_debit_card_account_payload_response_data)
print(open_debit_card_account_payload_response.status_code)

issue_virtual_card_payload = {
    "user_id": create_user_response_data['user']['id'],
    "account_id": open_debit_card_account_payload_response_data['account']['id']
}
